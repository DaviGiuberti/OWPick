...
#  Configuração
# ─────────────────────────────────────────────────────────────────────────────
# Região/tier padrão; podem ser sobrescritos por argv[2]/argv[3] (o app passa
# os valores do settings.json — tarefa 6.1).
REGION = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2].strip() else "AMER"
TIER = sys.argv[3] if len(sys.argv) > 3 and sys.argv[3].strip() else "GRANDMASTER_AND_CHAMPION"
...
# Mapas competitivos (nome, slug, modo) — vêm de owpick.core.heroes.
...
def load_heroes() -> dict:
    ...
    return {"version": "1.1.0", "patch": PATCH, "heroes": heroes.load_heroes_roles()}
...
# Função canônica de normalização de nomes (era uma cópia literal aqui).
...
def extract_via_network(intercepted: list[dict]) -> dict[str, dict]:
    ...
    def walk(obj):
        if isinstance(obj, dict):
            name = obj.get("heroName") or obj.get("name") or obj.get("hero") or obj.get("character")
...
def scrape_map(page, slug: str, name: str, intercepted: list[dict]) -> dict[str, dict]:
    ...
    try:
        ...
        log.warning(
            f"  {name}: '{READY_SELECTOR}' não apareceu (mapa recente ou sem dados suficientes)."
        )
        return {}
...
def run():
    ...
    # %APPDATA%\OWPick — tarefa 5.3); senão data/ do repositório (uso dev).
    ...
    # ── Grava maps.txt (apenas referência humana; não lido em runtime) ──
